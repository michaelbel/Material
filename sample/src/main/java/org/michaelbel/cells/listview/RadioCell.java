...
import org.michaelbel.material.widget2.RadioButton;
...
public class RadioCell extends BaseCell {
    ...
    public RadioCell(Context context) {
        ...
        radioButton.setLayoutParams(LayoutHelper.makeFrame(context, 22, 22, Gravity.START | Gravity.CENTER_VERTICAL, 16, 0, 16, 0));
        addView(radioButton);

        textView = new TextView(context);
        textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 16);
        textView.setTextColor(ContextCompat.getColor(context, R.color.textColorPrimary));
        textView.setLayoutParams(LayoutHelper.makeFrame(context, LayoutHelper.WRAP_CONTENT, LayoutHelper.WRAP_CONTENT, Gravity.START | Gravity.CENTER_VERTICAL, 54, 0, 0, 0));
        addView(textView);
    }
