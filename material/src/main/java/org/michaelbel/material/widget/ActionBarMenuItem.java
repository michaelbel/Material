...
public class ActionBarMenuItem extends FrameLayoutFixed {
    ...
    public ActionBarMenuItem addSubItem(int id, @StringRes int resId, int icon) {
        addSubItemPopup(id, getResources().getString(resId), icon);
        return this;
    }

    public ActionBarMenuItem addSubItem(int id, String text, int icon) {
        addSubItemPopup(id, text, icon);
        return this;
    }

    private TextView addSubItemPopup(int id, String text, int icon) {
        if (popupLayout == null) {
            rect = new Rect();
            location = new int[2];
