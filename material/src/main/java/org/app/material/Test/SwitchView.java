...
public class SwitchView extends View {
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		float sTop;
		float sLeft = sTop = 0;
		float sBottom = h * 0.91f;
		float sWidth = (float) w - sLeft;
		float sHeight = sBottom - sTop;
		sCenterX = ((float) w + sLeft) / 2;
		sCenterY = (sBottom + sTop) / 2;

		shadowHeight = h - sBottom;
		...
		sRectF.left = (float) w - sBottom;
		sRectF.right = (float) w;
